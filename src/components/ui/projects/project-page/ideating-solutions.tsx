...
type Props = {
    ...
    image2: string,
    body3: string,
    explorationImages?: { image: string; }[]
}
function IdeatingSolutions({ sectionHeading, title, body1, image1, body2, image2, body3, explorationImages }: Props) {
    return (
        <div className="flex flex-row gap-8 p-20" id="ideating-solutions">
                ...
                {explorationImages && (
                    <div className="flex flex-col gap-4 w-full">
                        {explorationImages.map((image, index) => (
                            <ImageOverlay key={index} src={image.image} alt="Problem Statement" width={1000} height={400} />
                        ))}
                    </div>
                )}
                <Body>{body1}</Body>
